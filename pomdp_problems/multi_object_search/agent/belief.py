...
class MosOOBelief(pomdp_py.OOBelief):
    ...
    def random(self, **kwargs):
        # print(pomdp_py.OOBelief.mpe(self, **kwargs))
        # print(pomdp_py.OOBelief.mpe(self, **kwargs).object_states)
        # print(MosOOState(pomdp_py.OOBelief.mpe(self, **kwargs).object_states))
        return MosOOState(pomdp_py.OOBelief.mpe(self, **kwargs).object_states)
...
def _initialize_histogram_belief(dim, robot_id, object_ids, prior, robot_orientations):
    ...
    """
    # prior = {}
    # prior[0] = ObjectState(0,'target',(1,0))
    # prior[1] = ObjectState(1,'target',(0,1))
    # prior[2] = ObjectState(2,'target',(2,1))
    # prior[3] = ObjectState(3,'target',(1,2))
...
def _initialize_particles_belief(dim, robot_id, object_ids, prior,
    ...
    width, length = dim
    for objid in object_ids:
        particles = [RobotState(robot_id, init_robot_pose, (),
                                None)]  # list of states; Starting the observable robot state.
        if objid in prior:
            # prior knowledge provided. Just use the prior knowledge
