...
class ParkOOPOMDP(pomdp_py.OOPOMDP):
    """
    A ParkOOPOMDP is instantiated given a string description
    of the search world, sensor descriptions for robots,
    and the necessary parameters for the agent's models.

    Simulates trashy park for clean up
...
def belief_update(agent, real_action, real_observation, next_robot_state, planner):
    ...
    # update to set only highest trash to be 1, all others to be 0
    goToList = {}
        ...
        for objid in agent.cur_belief.object_beliefs:
            ...
            if isinstance(belief_obj, pomdp_py.Histogram):
                if objid == agent.robot_id:
                    ...
                    #just set belief object before?
                    belief_obj = agent.trashList.getLocation()
                    agent.belief.object_beliefs[0]=belief_obj
                    ...
                    """if agent.trashList.maxTrash() != objid:
                        # for key in new_belief.histogram
                        # print(belief_obj)
                        # print(new_belief)
                        # print(new_belief.histogram)
                        # print(new_belief[objid][belief_obj])
                        for key in new_belief:
                            #new_belief.__delitem__()
                            #del new_belief[key]
                            print(key)
                            new_key = agent.trashList.getLocation()
                            #for key in new_key:
                            #    new_belief[new_key] = 1
                            new_belief = pomdp_py.update_histogram_belief(new_key,
                                                                          real_action,
...
def solve(problem,
    ...
    random_objid = list(problem.env.target_objects)[0]
    ...
    if visualize:
        viz = MosViz(problem.env,
                     controllable=False)  # controllable=False means no keyboard control.
    ...
    for i in range(max_steps):
        ...
        if isinstance(real_action, FindAction):
            # commented out to run for max_steps
            # _find_actions_count += 1
            problem.agent.trashList.printTrash()
        #removed prints for easier visibility
        #print("==== Step %d ====" % (i + 1))
...
def unittest():
    # random world
    #grid_map, robot_char = random_world(10, 10, 4, 0)
    grid_map, robot_char = park_world(5, 5, 4, 0)
    laserstr = make_laser_sensor(360, (1, 10), 0.5, False)
    proxstr = make_proximity_sensor(1, False)
    problem = ParkOOPOMDP(robot_char,  # r is the robot character
